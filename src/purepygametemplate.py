...
BOARD_RECT = pygame.Rect(
    BOARD_MARGIN, BOARD_MARGIN + MENU_HEIGHT, BOARD_WIDTH, BOARD_HEIGHT
)

# Dialog attributes
COLOR_DIALOG_BG = (50, 50, 60)
COLOR_DIALOG_BORDER = (150, 150, 150)
COLOR_BUTTON_BG = (100, 100, 110)
COLOR_BUTTON_HOVER_BG = (130, 130, 140)
...
class Gomoku:
    def __init__(self):
        ...
        self.font_dialog = pygame.font.SysFont("sans", 24)
    ...
    def setup_menus(self):
        menu_action_map = {
            ...
            "Help": [
                {"text": "Rule", "action": self.open_rules_dialog},
            ],
        }
        ...
        for dropdown_items in menu_action_map.values():
            for item in dropdown_items:
                item_width = self.font_small.render(
                    item["text"], True, COLOR_TEXT
                ).get_width()
        ...
        for menu_name in menu_action_map.keys():
            rect = pygame.Rect(
                x_pos,
                5,
                self.font_small.render(menu_name, True, COLOR_TEXT).get_width() + 20,
                30,
            )
    ...
    def open_rules_dialog(self):
        ...
        self.dialog_elements["bg_rect"] = pygame.Rect(
        ...
        self.dialog_elements["close_btn_rect"] = pygame.Rect(
    ...
    def draw_menus(self):
        ...
        for menu_name, menu_data in self.menus.items():
            if self.active_menu == menu_name or menu_data["hover"]:
                pygame.draw.rect(
                    self.screen, COLOR_MENU_HOVER_BG, menu_data["rect"], border_radius=4
                )
        ...
        if self.active_menu:
            ...
            dropdown_bg_rect = pygame.Rect(
                first_item_rect.left,
                first_item_rect.top,
                first_item_rect.width,
                last_item_rect.bottom - first_item_rect.top,
            )
            pygame.draw.rect(
                self.screen,
                COLOR_DROPDOWN_BG,
                dropdown_bg_rect.inflate(2, 2),
                border_radius=4,
            )
            ...
            for item in menu_data["items"]:
                if item["hover"]:
                    pygame.draw.rect(
                        self.screen, COLOR_MENU_HOVER_BG, item["rect"], border_radius=4
                    )
                text_surf = self.font_small.render(item["text"], True, COLOR_TEXT)
                text_rect = text_surf.get_rect(
                    centery=item["rect"].centery, left=item["rect"].left + 10
                )
                self.screen.blit(text_surf, text_rect)
    ...
    def draw_pieces(self):
        for r in range(GRID_SIZE):
            for c in range(GRID_SIZE):
                ...
                if player != 0:
                    pos = (
                        BOARD_RECT.left + c * CELL_SIZE,
                        BOARD_RECT.top + r * CELL_SIZE,
                    )
